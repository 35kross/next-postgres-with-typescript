...
import { IPost, IComment } from '../data-models';
...
class Post extends React.Component<IProps, IState> {
  ...
  _handleTitleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  render() {
    ...
    const maybeCommentElements = this.props.post.comments
      ...
      .map((c: IComment) => <CommentPreview key={`cmmt-${c.id}`} {...c} />);
...
export default withData({}, (state: IState) => state)(Post);
